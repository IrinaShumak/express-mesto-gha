  ...
  Card.findByIdAndRemove(req.params.cardId)
    ...
    .catch((err) => {
      if (err.name === 'CastError') {
        res.status(ERROR_CODE_MISSING_ENTRY).send({ message: 'Карточка с указанным _id не найдена.' });
        return;
      }
      res.status(ERROR_CODE_OTHER).send({ message: 'Произошла ошибка' });
    });
...
module.exports.likeCard = (req, res) => {
  Card.findByIdAndUpdate(
    ...
    .catch((err) => {
      ...
      if (err.name === 'CastError') {
        res.status(ERROR_CODE_MISSING_ENTRY).send({ message: 'Передан несуществующий _id карточки.' });
        return;
      }
      res.status(ERROR_CODE_OTHER).send({ message: 'Произошла ошибка' });
    });
